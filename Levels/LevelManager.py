...
class LevelManager():
    ...
    def next_unfinished_sublevel(self, index=0):
        def next_unfinished_sublevel(self, index=0):
            ...
            if self.p is None or self.p.playerAnte not in self.levelsDict:
                return None

            sublevels = self.levelsDict[self.p.playerAnte]


            if index >= len(sublevels):
                return None


            if not sublevels[index].finished:
